...
#### Provide analysis of Bellabeat competitor products (like Fitbit, Applewatch, etc.) health data in order to enable the marketing team with more targeted campaign strategies.
...
Data set suggested by Srsen - contains two separate directories that have data describing FitBit user activity for March through April, and April through May.  There is some consistency to the metrics measured.  We will import this directly from the website into our RStudio workbook 'dataset' directory and process the data for analysis from there.
...
Apple's health app data, this dataset has a number of aspects associated with health metrics tracked through apples health applications.  
...
#### **Reason for using R**
I used R for every step of this project because I am already fairly well versed with Alteryx, Tableau and SQL from prior work experience.


### Preparation of the data

The data has been input into this Rstudio workbook and is ready to be investigated further to assisst Srsen with her theory and analysis.
...
4. Determine the credibility of the data.
   ...
   B.  The Soya pillow study data is a little limited content wise, but the data is very well populated so it will be a good set to drive insights from.
   ...
   F.  Also worth noting, that for the purpose of the course and the project timelines, I limited my data resourcing to just Kaggle.  I'm certain there is pertinent information available in the BigQuery public datasets.  


### Preparing the Data for Analysis

With all the data we have, I believe cherry picking the ones that seem most relevant to activity that would be conducted by Bellabeat product users is best.  At this point I would have spoken with Srsen and provided her with the list of additional data sets I found at her request, then,  Based on her feedback I would prioritize analysis of the data sets that seemed most appealing to her subject matter expertise, as to not waste time.  However, given that I don't have that capability, I used my best judgment.  

Initially, we eliminate the 'Apple Health' data set due to inefficient population sampling. 

Looking at FitBit data, we're going to start with our daily activity files from both months to get a good idea of what fitbit activity data looks like in general.  This may open the door for additional anlysis in other activity-type trends and correlation.  From there, we'll include hourly calories, intensities, and sleep data as well to support the health app-data theories we want to look into (described below).  We will also bring in weight log, as losing weight (or fat, rather) is a largely appealing result of tracking your fitness activities.  With BMI we should be able to see what individuals with high activity and good sleep patterns can achieve.

Our 'health-app-data' dataset contains some data that may be useful while looking at the fitbit dataset.  We are going to take active energy burned and headphone audio to see if there's any correlation with music and intensity of activity.  We will also take a look at the sleep analysis data available to support another theory about stress and sleep in the marketing demographic, and utilize this in conjunction with sleep data from our fitbit dataset and SoYa stress/sleep analysis dataset.

Lastly, we're going to take a look at the SoYa smart pillow sleep analysis dataset.  There are translations for the acronyms within the dataset available, so we're going to update the data frame accordingly.
